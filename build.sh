...
if g++ -std=c++14 egtNetwork.cpp -o egtNetwork_x86 -O $WARN_CFLAGS `pkg-config libegt --cflags --libs` -pthread ; then
...
# Build for ARM
if $CXX $INC_DIR egtNetwork.cpp -o egtNetwork_ARM -O $WARN_CFLAGS `pkg-config libegt --cflags --libs` -pthread; then
